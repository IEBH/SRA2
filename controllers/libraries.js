...
/**
* Compare several library contents against one another
* @param array req.body.libraries The additional libraries to compare the first one against
*/
app.post('/api/libraries/:id/compare', function(req, res) {
	async()
		.then(function(next) {
			// Sanity checks {{{
			if (!req.params.id) return next('id must be specified');
			if (!req.user) return next('You must be logged in');
			if (!req.body.libraries) return next('No additional libraries specified');
			next();
			// }}}
		})
		.then('libraries', function(next) {
			...
			var libraries = [
				{_id: req.params.id, library: null, references: null},
			];
			this.body.libraries.forEach(function(id) {
		...
		.then('mergedRefs', function(next) {
			// Process all mismatching items {{{
			var self = this;
			var mergedRefs = [];
			var libA = self.libraries[0];
			self.libraries[0].references.forEach(function(refA) {
				var conflicts = {};
				conflicts[libA._id] = {};
				...
				self.libraries.slice(1).forEach(function(libB) {
					...
					var refB = _.find(otherLib.references, {_id: ref._id});
					if (!refB) {
						...
						} else if ((!ref.tags || !ref.tags.length) && otherRef.tags && otherRef.tags.length) { // B has tags, A does not
